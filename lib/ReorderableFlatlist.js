...
export const ReorderableFlatList = (props) => {
    ...
    const renderItem = useCallback(({ item }) => {
        ...
        return (<ListItem ref={handleRef} item={item} 
        // @ts-ignore
        onMoveUp={moveUp} 
        // @ts-ignore
        onMoveDown={moveDown}/>);
    ...
    return (<View>
      <FlatList data={data} renderItem={renderItem} ItemSeparatorComponent={() => <View style={{ height: 10 }}/>} keyExtractor={(item) => props.keyExtractor(item)}/>
